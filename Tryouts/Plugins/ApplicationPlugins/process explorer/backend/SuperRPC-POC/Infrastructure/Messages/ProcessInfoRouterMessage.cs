...
public class ProcessInfoRouterMessage : IObserver<TopicMessage>
{
    private readonly ILogger<ProcessInfoRouterMessage> _logger;
    private readonly IProcessInfoAggregator? _processInfoAggregator;

    public ProcessInfoRouterMessage(ILogger<ProcessInfoRouterMessage>? logger,
        IProcessInfoAggregator? processInfoAggregator)
    {
        _logger = logger ?? NullLogger<ProcessInfoRouterMessage>.Instance;
    ...
    public void OnNext(TopicMessage value)
    {
        ...
        if (processInfo.Equals(default))
        {
            return;
        }

        Thread.Sleep(5000);
        //_processInfoAggregator?.AddProcessInfo(processInfo);
    }
}

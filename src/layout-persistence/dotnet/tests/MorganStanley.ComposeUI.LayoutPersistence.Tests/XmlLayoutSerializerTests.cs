...
{
    private readonly XmlLayoutSerializer<LayoutData> _serializer = new();
    ...
    public async Task Serialize_Then_Deserialize_ShouldReturnTheOriginalObject()
    {
        ...
        var xml = await _serializer.SerializeAsync(layoutObject);
        
        var loadedData = await _serializer.DeserializeAsync(xml);

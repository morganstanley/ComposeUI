...
internal class Fdc3DesktopAgent : IFdc3DesktopAgentBridge
{
    ...
    public async ValueTask<RaiseIntentResult<RaiseIntentResponse>> RaiseIntentForContext(RaiseIntentForContextRequest request, string contextType)
    {
        ...
        //TODO: Decide if we want to allow apps to raise intent if they are registering their intents into the Fdc3App.Interop.Intents.Raises collection
        if (!sourceApp.CanRaiseIntent(contextType: contextType))
        {
            if (_logger.IsEnabled(LogLevel.Warning))
            {
                _logger.LogWarning($"Source app did not registered its intent for context: {contextType} in the `raises` section!");
            }
        }
    ...
    public async ValueTask<RaiseIntentResult<RaiseIntentResponse>> RaiseIntent(RaiseIntentRequest request, string contextType)
    {
        ...
        if (!sourceApp.CanRaiseIntent(request.Intent, contextType))
        {
            if (_logger.IsEnabled(LogLevel.Warning))
            {
                _logger.LogWarning($"Source app did not registered its intetnt to raise intents for context: {contextType} and intent: {request.Intent}, this combination of settigns cannot be found in the `raises` section!");
            }
        }

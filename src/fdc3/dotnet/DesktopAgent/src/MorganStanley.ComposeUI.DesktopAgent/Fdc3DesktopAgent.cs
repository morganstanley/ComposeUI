...
internal class Fdc3DesktopAgent : IFdc3DesktopAgentBridge
{
    ...
    public async ValueTask<OpenResponse?> Open(OpenRequest? request)
    {
        ...
        try
        {
            ...
            //TODO: context deserializing
            var context = JsonSerializer.Deserialize<Context>(request.Context, new JsonSerializerOptions(JsonSerializerDefaults.Web));

            var cancellationToken = new CancellationToken();
            var listener = await GetContextListener(targetInstanceId, context!.Type, cancellationToken).WaitAsync(_options.ListenerRegistrationTimeout, cancellationToken);

...
internal class ResolverUIMessageRouterCommunicator : IResolverUICommunicator
{
    ...
    public ResolverUIMessageRouterCommunicator(
        ...
        ILogger<ResolverUIMessageRouterCommunicator>? logger)
    {
        _messageRouter = messageRouter;
        _logger = logger ?? NullLogger<ResolverUIMessageRouterCommunicator>.Instance;
    }

    public async Task<ResolverUIResponse?> SendResolverUIRequest(IEnumerable<IAppMetadata> appMetadata, TimeSpan? timeout = null)
    {
        using var cancellationTokenSource = new CancellationTokenSource(timeout ?? _defaultTimeout);
        try
        {
            return await SendResolverUIRequestCore(appMetadata, cancellationTokenSource.Token);
        }
    ...
    public async Task<ResolverUIIntentResponse?> SendResolverUIIntentRequest(IEnumerable<string> intents, TimeSpan? timeout)
    {
        ...
        try
        {
            return await SendResolverUIIntentRequestCore(intents, cancellationTokenSource.Token);
        }
        catch (TimeoutException ex)
        {
            if (_logger.IsEnabled(LogLevel.Debug))
